...
This directory contains the repair-development lab for `fsck.nextufs.modern`.
...
```sh
make -C fsck.modern -f Makefile.linux repair-lab
fsck.modern/tests/scripts/run_case.sh legacy -n bad-block-count
fsck.modern/tests/scripts/run_case.sh modern -n bad-block-count
fsck.modern/tests/scripts/compare_case.sh -n bad-block-count
fsck.modern/tests/scripts/compare_all_cases.sh -n
```
...
Current limitation:
- the `lostfound-*` fixtures are staged but not yet clean oracle cases
  - they still trigger enough surrounding fallout that they do not yet isolate
    the exact legacy `lost+found` create/reallocate repair paths cleanly
- true mutating repair-oracle runs are currently blocked on this host
  - both legacy `fsck.nextufs` and `fsck.nextufs.modern` detect byte-swapped
    NextStep UFS and intentionally drop write access in `setup()`
  - that means `reply()` falls back to `no` even under `-y`, so repair-path
    prompts cannot yet be driven to actual on-disk mutation on these raw images
