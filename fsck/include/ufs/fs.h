...
/* BEGIN CS_RPAUSE */
	 ...
	 */
	union {
	    int32_t sufs_sparecon[5];	/* reserved for future constants */
	} fs_spareun;
...
/* these fields retain the current block allocation info */
	...
	int32_t fs_csp_pad[MAXCSBUFS];	/* on-disk placeholders */
	int32_t	fs_cpc;			/* cyl per cycle in postbl */
	int16_t	fs_postbl[MAXCPG][NRPOS];/* head of blocks for each rotation */
	int32_t	fs_magic;		/* magic number */
	uint8_t	fs_rotbl[1];		/* list of blocks for each rotation */
/* actually longer */
};

extern struct csum *fsck_fs_csp[MAXCSBUFS];
/*
...
#define fs_cs(fs, indx) \
	fsck_fs_csp[(indx) >> (fs)->fs_csshift][(indx) & ~(fs)->fs_csmask]
...
struct	cg {
	int32_t	cg_link;		/* on-disk placeholder */
	int32_t	cg_rlink;		/* on-disk placeholder */
